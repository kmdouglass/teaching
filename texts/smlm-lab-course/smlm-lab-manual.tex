...
In this model, a single pixel takes an input (a number of photons) and transforms this number into an output (a digital value in analog-to-digital units, or ADU). In performing this transformation, various sources of noise are added to the signal so that we can only make probabilisitic inferences about the actual number of photons that hit the pixel. There are also a few assumptions in this model that, when taken together, constitute what's called an ideal image sensor. They are:

\begin{enumerate}
    \item The numer of photons collected by the pixel depends on the product of irradiance $I \, \left[ W / m^2 \right]$ and the exposure time $t_{exp} \, \left[s\right]$.
    \item The sensor is linear, i.e. the digital signal $y$ increases linearly with the number of photons received.
    \item All noise sources are constant in time and space.
    \item Only the quantum efficiency of the camera is dependent on the wavelength of the incident irradiation.
\end{enumerate}

From these five assumptions, the laws of quantum mechanics, and the laws of probability, expressions for the mean output, $\mu_y$, the variance of the output $\sigma^2_y$, and the signal-to-noise ratio $\text{SNR}$ may be derived.

\begin{align}
    \mu_y &= K \mu_d + K \eta \mu_p + B\\
    \sigma_y^2 &= K^2 \sigma_d^2 + K \left( \mu_y - K \mu_d - B\right) \\
    \text{SNR} &= \frac{\eta \mu_p}{\sqrt{\sigma_d^2 + \eta \mu_p}}
\end{align}

In these expressions, $\mu$ and $\sigma^2$ represent the mean and variance of the number of photons hitting the pixel ($p$), the dark noise ($d$), and the output signal in ADU's ($y$). The number of electrons $\mu_e$ generated by $\mu_p$ photons hitting the active area of the sensor is obtained from the expression for the quantum efficiency $\eta$, a property of the pixel that denotes how efficiently photons are converted into photoelectrons. In general, the quantum efficiency depends on the wavelength:

\begin{equation}
    \eta \left( \lambda \right) = \frac{\mu_e}{\mu_p}
\end{equation}

$K$ is referred to as the overall system gain (or more simply, the gain). It has units of $\frac{\text{ADU}}{e^-}$ and represents the amplification of the signal in photoelectrons into ADUs. In microscopy, the dark noise $\sigma_d^2$ is dominated by the read noise, which comes from a random value that is added onto the signal when the voltage is read out from the pixel. $B$, in units of $\text{ADU}$ is called the offset and is a constant value that's added onto the output of a pixel to make sure that its value never drops below 0.

The most important part of the above model is the behavior of the pixel when the light signal is much stronger than the read noise. In this case, the number of detected photons $\eta \mu_p$ is much greater than the read noise $\sigma_d^2$. The signal-to-noise ratio is then approximately $\text{SNR} \sim \sqrt{\eta \mu_p}$. This is the so-called photon shot noise limit, and it is the best that we can do when measuring the number of photons from a molecule.
...
\begin{equation}
    f \left(x, y\right) = a \exp \left[ \frac{\left(x-x_{0}^2\right) + \left(y - y_{0}^2\right)}{2 w^2} \right] + b
\end{equation}
...
\subsubsection{Gaussian vs. Airy Disk PSFs}

As shown in \autoref{fig:airy-vs-gauss}, a Gaussian model can be a very good approximation to an Airy disk. In fact, this close similarity and the computational efficiency of fitting a Gaussian is why it is almost always chosen over the Airy disk as the PSF model in SMLM. However, there are some important differences between the two models that are worth noting.

For one, the PSF width is often stated as $\Delta r = 0.61 \lambda / \text{NA} $. This is the distance between the central peak and the first zeros of the Airy disk. The width of the Gaussian PSF, on the other hand, can be described by the $w$ parameter. The two measures of PSF width are not the same thing. However, we can still determine a relationship between $w$, $\lambda$, and $\text{NA}$, which is\cite{zhang-appliedoptics-2007}:

\begin{equation}
    w = 0.21 \frac{\lambda}{\text{NA}}
\end{equation}
